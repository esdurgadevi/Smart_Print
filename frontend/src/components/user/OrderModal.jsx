...
const OrderModal = ({ isOpen, onClose, shopId, service }) => {
  ...
  const costPerPage = parseFloat(service.price);

  const calculatePagesToPrint = () => {
    if (!pageRange || pageRange.trim() === "all" || pageRange.trim() === "All" || pageRange.trim() === "") {
      return pageCount || 1; // if it's not a pdf, maybe it's 1 page
    }
  ...
  const totalAmount = currentPagesToPrint * copies * costPerPage;
        ...
        <div className="p-6 overflow-y-auto">
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="bg-gray-900 rounded-2xl p-5 text-white flex justify-between items-center shadow-lg shadow-gray-200">
                <div>
                  <span className="block text-gray-400 text-sm font-medium mb-1">Total Estimated Cost</span>
              ...
              <button 
                ...
                className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold text-lg py-4 rounded-2xl shadow-lg shadow-orange-200 transition-all active:scale-[0.98] flex justify-center items-center gap-2"
              >
                Add to Cart
              </button>
